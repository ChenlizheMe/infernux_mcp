...
from infernux_mcp.tools.common import fail, ok, register_tool_metadata
from infernux_mcp.tools.runtime import MotionCaptureStopMode, RuntimeAssertion


_CURRENT_SUPERVISOR: SupervisorSession | None = None
...
def _supervisor() -> SupervisorSession:
    ...
    cached = _CURRENT_SUPERVISOR
    if (
        cached is not None
        and cached.session_id == active.session_id
        and cached.project_root == active.project_root
    ):
        return cached
    _CURRENT_SUPERVISOR = SupervisorSession.attach_current_host(
        active.project_root,
        active.session_id,
        mode=active.mode,
